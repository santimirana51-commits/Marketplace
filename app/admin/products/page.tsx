...
export default async function AdminProducts() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <table className="table mt-6"><thead><tr><th>Title</th><th>Slug</th><th>Status</th><th></th></tr></thead>
        <tbody>
          {(products ?? []).map((p) => {
            const pr = p as { id: string; title: string; slug: string; status: string };
            return <tr key={pr.id}><td>{pr.title}</td><td className="text-zinc-500">{pr.slug}</td><td>{pr.status}</td><td><Link href={`/admin/products/${pr.id}`} className="underline">Edit</Link></td></tr>;
          })}
        </tbody>
      </table>
    </div>
